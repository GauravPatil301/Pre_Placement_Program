...
function findCommonElements(arr1, arr2, arr3) {
  ...
  while (i < arr1.length && j < arr2.length && k < arr3.length) {
    ...
    if (arr1[i] === arr2[j] && arr2[j] === arr3[k]) {
      result.push(arr1[i]);  // Add the common element to the result array
      i++;  // Move to the next element in arr1
      j++;  // Move to the next element in arr2
      k++;  // Move to the next element in arr3
    }
    // If the current element is smaller than any of the other arrays,
    // move to the next element in that particular array
...
function findDistinctIntegers(nums1, nums2) {
  ...
  const result = [
    [...set1].filter(num => !set2.has(num)),  // Find distinct integers in nums1 not present in nums2
    [...set2].filter(num => !set1.has(num))   // Find distinct integers in nums2 not present in nums1
  ];
...
function transposeMatrix(matrix) {
  ...
  const transposedMatrix = new Array(cols).fill(null).map(() => new Array(rows));

  // Traverse the original matrix and fill the transposed matrix
...
function arrayPairSum(nums) {
  ...
  // Iterate over the sorted array and sum the minimum element of each pair
...
function sortedSquares(nums) {
  const squared = nums.map(num => num * num);  // Square each number
  squared.sort((a, b) => a - b);  // Sort the squared values in non-decreasing order
  return squared;
}
